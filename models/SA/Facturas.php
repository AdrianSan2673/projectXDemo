...
class Facturas
{
	...
	public function getFecha_cancelacion()
	{
		return $this->fecha_cancelacion;
	}

	public function setFecha_cancelacion($fecha_cancelacion)
	{
	...
	public function update()
	{
		...
		$fecha_cancelacion = $this->getFecha_cancelacion();

		$stmt = $this->db->prepare("UPDATE rh_Candidatos_Facturas SET Tipo=:Tipo,Folio_Factura=:Folio_Factura, Fecha_Emision=:Fecha_Emision, Razon_Social=:Razon_Social, Estado=:Estado, Promesa_Pago=:Promesa_Pago, Monto=:Monto, Monto_IVA=:Monto_IVA, Fecha_de_Pago=:Fecha_de_Pago,fecha_cancelacion=:fecha_cancelacion WHERE Folio_Factura=:Folio");
		...
		$stmt->bindParam(":fecha_cancelacion", $fecha_cancelacion, PDO::PARAM_STR);
	...
	public function updateFollowUp()
	{
		...
		if ($Estado == 'Pagada') {
			...
			$stmt = $this->db->prepare("UPDATE rh_Candidatos_Facturas SET Estado=:Estado, Promesa_Pago=:Promesa_Pago, Proxima_Gestion=:Proxima_Gestion,fecha_cancelacion=:fecha_cancelacion WHERE Folio_Factura=:Folio_Factura");
		}
	...
	public function getFacturasPendientes()
	{
       ...
       FROM rh_Candidatos_Facturas CF INNER JOIN rh_Ventas_Alta V ON CF.ID_Cliente=V.Cliente INNER JOIN rh_Ventas_Empresas E ON V.Empresa=E.Empresa WHERE Estado='Pendiente de pago' AND Tipo<>1 ORDER BY Fecha_Emision ASC");
		$stmt->execute();
	...
	{
		$stmt = $this->db->prepare("SELECT TOP(1000) CF.*, DATEDIFF(DAY, Fecha_Emision, GETDATE()) AS Dias_Transcurridos, E.Nombre_Empresa,
        (SELECT top 1 Comentarios FROM rh_Candidatos_Facturas_Gestiones WHERE Folio_Factura=CF.Folio_Factura order by Fecha desc) as Ultima_Gestion,
        (SELECT top 1 Fecha FROM rh_Candidatos_Facturas_Gestiones WHERE Folio_Factura=CF.Folio_Factura order by Fecha desc) as Fecha_Ultima_Gestion,
         (SELECT Dias_Credito FROM rh_Ventas_Alta WHERE Cliente=CF.ID_Cliente) as Plazo_Credito
	   FROM rh_Candidatos_Facturas CF INNER JOIN rh_Ventas_Alta V ON CF.ID_Cliente=V.Cliente INNER JOIN rh_Ventas_Empresas E ON V.Empresa=E.Empresa WHERE Estado='Pagada' AND YEAR(Fecha_Emision) = YEAR(GETDATE())  ORDER BY Fecha_Emision DESC");
		$stmt->execute();
		$facturas = $stmt->fetchAll();
		return $facturas;
	}
	...
	public function getFacturasIncobrables()
	{
		$stmt = $this->db->prepare("SELECT CF.*, DATEDIFF(DAY, Fecha_Emision, GETDATE()) AS Dias_Transcurridos, E.Nombre_Empresa,
        (SELECT top 1 Comentarios FROM rh_Candidatos_Facturas_Gestiones WHERE Folio_Factura=CF.Folio_Factura order by Fecha desc) as Ultima_Gestion,
        (SELECT top 1 Fecha FROM rh_Candidatos_Facturas_Gestiones WHERE Folio_Factura=CF.Folio_Factura order by Fecha desc) as Fecha_Ultima_Gestion,
         (SELECT Dias_Credito FROM rh_Ventas_Alta WHERE Cliente=CF.ID_Cliente) as Plazo_Credito
	   FROM rh_Candidatos_Facturas CF INNER JOIN rh_Ventas_Alta V ON CF.ID_Cliente=V.Cliente INNER JOIN rh_Ventas_Empresas E ON V.Empresa=E.Empresa WHERE Tipo=1 ORDER BY Fecha_Emision DESC");
		$stmt->execute();
		$facturas = $stmt->fetchAll();
		return $facturas;
	}
	...
	public function updateInfoCancelados()
	{
		...
		$stmt = $this->db->prepare("UPDATE rh_Candidatos_Facturas SET  comentarios=:comentarios,fecha_cancelacion=:fecha_cancelacion WHERE Folio_Factura=:Folio_Factura");
		$stmt->bindParam(":Folio_Factura", $Folio_Factura, PDO::PARAM_STR);
	...
	public function getFacturasPagadasPorFecha()
	{
        ...
        (SELECT top 1 Fecha FROM rh_Candidatos_Facturas_Gestiones WHERE Folio_Factura=CF.Folio_Factura order by Fecha desc) as Fecha_Ultima_Gestion,
         (SELECT Dias_Credito FROM rh_Ventas_Alta WHERE Cliente=CF.ID_Cliente) as Plazo_Credito
	   FROM rh_Candidatos_Facturas CF INNER JOIN rh_Ventas_Alta V ON CF.ID_Cliente=V.Cliente INNER JOIN rh_Ventas_Empresas E ON V.Empresa=E.Empresa WHERE Estado='Pagada'  AND Fecha_Emision BETWEEN :fecha_inicio AND :fecha_fin ORDER BY Fecha_Emision DESC");
