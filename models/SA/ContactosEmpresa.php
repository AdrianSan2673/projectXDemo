...
class ContactosEmpresa
{
    ...
    public function getContactosPorEmpresa()
    {
        ...
        $stmt = $this->db->prepare("SELECT ac.*, ac.ID AS ID_Contacto, ct.nombre_tipo as nombre_tipo,u.password FROM rh_Ventas_Alta_Contactos ac INNER JOIN rh_ventaS_Contacto_tipo ct ON ac.tipo_usuario=ct.id INNER JOIN reclutamiento2.dbo.users u on ac.Usuario=u.username WHERE Empresa=:Empresa AND Activo=1 ORDER BY Nombre_Contacto");
