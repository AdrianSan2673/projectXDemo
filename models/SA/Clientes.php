...
class Clientes
{
	...
	public function getNotasPorEmpresa()
	{
		...
		$stmt = $this->db->prepare("SELECT *  FROM Clientes_Notas n  INNER JOIN rrhhinge_Candidatos.dbo.rh_Ventas_Alta va ON va.Cliente= n.ID_Cliente WHERE va.empresa=:empresa ORDER BY Fecha DESC");
		$stmt->bindParam(":empresa", $empresa, PDO::PARAM_INT);
		$stmt->execute();
		$fetch = $stmt->fetchAll();
		return $fetch;
