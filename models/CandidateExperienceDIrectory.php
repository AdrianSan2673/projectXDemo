...
class CandidateExperienceDirectory
{
    private $id;
    private $position;
    private $enterprise;
    ...
    public function getExperiencesByCandidate()
    {
        ...
        $stmt = $this->db->prepare("SELECT *, ce.id AS id_experience,ce.start_date,ce.end_date FROM candidate_experience_directory ce INNER JOIN states s ON ce.id_state=s.id INNER JOIN cities ct ON ce.id_city=ct.id WHERE id_directory=:id_directory ORDER BY ce.id, ce.still_works DESC, ce.end_date DESC, ce.start_date DESC");
        $stmt->bindParam(":id_directory", $id_directory, PDO::PARAM_STR);
        $stmt->execute();
    ...
    public function save()
    {
        ...
        $stmt = $this->db->prepare("INSERT INTO candidate_experience_directory(id_directory, position, enterprise, id_area, id_subarea, id_state, id_city, start_date, end_date, still_works, review, activity1, activity2, activity3, activity4,type) VALUES (:id_directory, :position, :enterprise, :id_area, :id_subarea, :id_state, :id_city, :start_date, :end_date, :still_works, :review, :activity1, :activity2, :activity3, :activity4,:type)");
    ...
    public function update()
    {
        ...
        $stmt = $this->db->prepare("UPDATE candidate_experience_directory SET position=:position, enterprise=:enterprise, id_area=:id_area, id_subarea=:id_subarea, id_state=:id_state, id_city=:id_city, start_date=:start_date, end_date=:end_date, still_works=:still_works, review=:review, activity1=:activity1, activity2=:activity2, activity3=:activity3, activity4=:activity4 WHERE id=:id");
    ...
    {
        $id = $this->getId();
        $stmt = $this->db->prepare("SELECT * FROM candidate_experience_directory  WHERE id=:id ");
        $stmt->bindParam(":id", $id, PDO::PARAM_INT);
        $stmt->execute();
    ...
    {
        $id = $this->getId();
        $stmt = $this->db->prepare("DELETE TOP (1) FROM candidate_experience_directory WHERE id=:id");
        $stmt->bindParam(":id", $id, PDO::PARAM_INT);
        $flag = $stmt->execute();
        if ($flag) {
            $result = true;
    ...
    {
        $id_directory = $this->getId_directory();
        $stmt = $this->db->prepare("DELETE  FROM candidate_experience_directory WHERE id_directory=:id_directory and type='operativa'");
        $stmt->bindParam(":id_directory", $id_directory, PDO::PARAM_INT);
        $flag = $stmt->execute();
        if ($flag) {
            $result = true;
    ...
    {
        $id_directory = $this->getId_directory();
        $stmt = $this->db->prepare("SELECT * FROM candidate_experience_directory  WHERE id_directory=:id_directory ");
        $stmt->bindParam(":id_directory", $id_directory, PDO::PARAM_INT);
        $stmt->execute();
