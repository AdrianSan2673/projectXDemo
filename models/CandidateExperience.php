...
class CandidateExperience
{
	...
	public function save()
	{
		...
		$stmt = $this->db->prepare("INSERT INTO candidate_experience(id_candidate, position, enterprise, id_area, id_subarea, id_state, id_city, start_date, end_date, still_works, review, activity1, activity2, activity3, activity4,type) VALUES (:id_candidate, :position, :enterprise, :id_area, :id_subarea, :id_state, :id_city, :start_date, :end_date, :still_works, :review, :activity1, :activity2, :activity3, :activity4,:type)");
	...
	{
		$id_candidate = $this->getId_candidate();
		$stmt = $this->db->prepare("DELETE  FROM candidate_experience WHERE id_candidate=:id_candidate and type='operativa'");
		$stmt->bindParam(":id_candidate", $id_candidate, PDO::PARAM_INT);
		$flag = $stmt->execute();
		if ($flag) {
			$result = true;
