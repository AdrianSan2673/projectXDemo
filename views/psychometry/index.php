                    ...
                    <tbody>
                        ...
                        <tr>
                            <?php switch ($psycho['status']) {
                                ...
                                ?>
                            <td class="text-center align-middle"><?= Utils::getShortDate($psycho['request_date']) ?>
                            </td>
                            <td class="text-center align-middle">
                                <?= $psycho['first_name'] . ' ' . $psycho['surname'] . ' ' . $psycho['last_name'] ?>
                            </td>
                            ...
                            <td class="text-center align-middle">
                                <?= !is_null($psycho['end_date']) && !empty($psycho['end_date']) ? $psycho['end_date'] : '' ?>
                            </td>
                            <td class="text-center py-0 align-middle">
                                <div class="btn-group btn-group-sm">
                                    <!-- gabo22 -->
                                    <?php if (Utils::isAdmin() || Utils::isManager() || Utils::isSales() || Utils::isSalesManager()) : ?>
                                    <a href="<?= base_url ?>psicometria/editar&id=<?= Encryption::encode($psycho['id']) ?>"
                                        class="btn btn-info" target="blank_">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <?php endif ?>
                                    <!-- gabo22 -->
                                    <?php if ($routeDocu != false) : ?>
                                    <a href="<?= $routeDocu ?>" class="btn btn-success" target="blank_"
                                        title="Ver Psicometria">
                                        <i class="fas fa-file-pdf"></i>
                                    </a>

                                    <?php endif;  ?>

                                    <a href="<?= base_url ?>psicometria/ver&id=<?= Encryption::encode($psycho['id']) ?>"
                                        class="btn btn-success" target="blank_">
