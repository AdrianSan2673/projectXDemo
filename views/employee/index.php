    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12">
                    <div
                        class="alert <?= $_SESSION['identity']->username == 'salmaperez' ? 'alert-maroon' : 'alert-success' ?>">
                        <h3>Plantilla de <?= Encryption::decode($_GET['flag']) == 1 ? 'empleados' : 'exempleados'  ?>
                        </h3>
                    </div>
                </div>
            </div>
    ...
    <section class="content-header">
        <div class="row">
            <div class="col-3  ml-auto">
                <a href="<?= base_url ?>reporte/employeesinformation&status=<?= $_GET['flag'] ?>"
                    class="btn btn-success ">Excel de empleados <br><i class="fas fa-file-excel display-4"></i></a>
            </div>
            <div class="col-3  ml-auto">
                <a href="<?= base_url ?>reporte/ExcelPAsswords" class="btn btn-primary ">Excel Contraseñas empleados
            ...
            <div class="col-2 ml-auto" <?= Encryption::decode($_GET['flag']) == 1 ? '' : 'hidden'  ?>>
                <div class="btn-group mr-3 text-center">
                    <button class="btn btn-orange dropdown-toggle dropdown-icon" data-toggle="dropdown">Nuevo empleado
                    </button>
        ...
        <div class="card bg-transparent">
            <div class="card-header">
                <h3 class="card-title">Listado de
                    <?= Encryption::decode($_GET['flag']) == 1 ? 'empleados' : 'exempleados'  ?> </h3>
            </div>
                ...
                <table id="tb_employees" class="table table-responsive table-striped table-sm" style="display: none;">
                    ...
                    <tbody id="tb_employees_body">
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle"><?= Utils::getFullDate($employee['modified_at']) ?>
                            </td>
                            <td class="text-center align-middle">
                                <a href="<?= base_url ?>empleado/ver&id=<?= Encryption::encode($employee['id_employee']) ?>"
                                ...
                                <button class="btn btn-danger"
                                    value="<?= Encryption::encode($employee['id_employee']) ?>"
                                    name="<?= $employee['first_name'] . ' ' . $employee['surname'] . ' ' . $employee['last_name'] ?>">Borrar</button>
                                <?php endif; ?>
