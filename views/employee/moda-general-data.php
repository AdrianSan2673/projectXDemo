                ...
                <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_gender" id="id_gender" class="form-control" required>
                                    ...
                                    <option value="<?= $gender['id'] ?>"
                                        <?= isset($employee) && is_object($employee) && $gender['id'] == $employee->id_gender ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group col-6">
                            ...
                            <select name="civil_status" class="form-control">
                                ...
                                <option value="Casado(a)"
                                    <?= $employee->civil_status == 'Casado(a)' ? 'selected' : ''; ?>>Casado(a)</option>
                                <option value="Divorciado(a)"
                                    <?= $employee->civil_status == 'Divorciado(a)' ? 'selected' : ''; ?>>Divorciado(a)
                                </option>
                                <option value="Soltero(a)"
                                    <?= $employee->civil_status == 'Soltero(a)' ? 'selected' : ''; ?>>Soltero(a)
                                </option>
                                <option value="Union libre"
                                    <?= $employee->civil_status == 'Union libre' ? 'selected' : ''; ?>>Union libre
                                </option>
                                <option value="Viudo" <?= $employee->civil_status == 'Viudo' ? 'selected' : ''; ?>>Viudo
                                </option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="form-group col-6">
                            ...
                            <input type="text" name="curp" value="<?= isset($employee->curp) ? $employee->curp : ''  ?>"
                        ...
                        <div class="form-group col-6">
                            ...
                            <input type="number" name="employee_number" minlength="0" class="form-control"
                                value="<?= isset($employee->employee_number) ?  $employee->employee_number : '' ?>">
                        </div>
                    ...
                    <div class="row">
                        <div class="form-group col-6">
                            ...
                            <input type="text" name="nss" value="<?= isset($employee->nss) ? $employee->nss : ''  ?>"
                                class="form-control" maxlength="11">
                        </div>

                        <div class="form-group col-6">
                            <label class="col-form-label">RFC</label>
                            <input type="text" name="rfc" value="<?= isset($employee->rfc) ? $employee->rfc : ''  ?>"
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-6">
                            ...
                            <select name="cliente" id="Cliente" class="form-control" required>
                                ...
                                <option value="<?= $contacto['Cliente'] ?>"
                                    <?= $employee->Cliente == $contacto['Cliente'] ? 'selected' : ''  ?>>
                        ...
                        <div class="form-group col-6">
                            ...
                            <select class="form-control" name="id_razon" id="select_razon_social" required>
                                ...
                                <option value="<?= $razson['ID_Razon'] ?>"
                                    <?= $employee->id_razon == $razson['ID_Razon'] ? 'selected' : ''  ?>>
                                    <?= $razson['Razon'] ?></option>
                                <?php endforeach ?>
                                <?php endif; ?>
                            </select>
                    ...
                    <div class="form-group ">
                        ...
                        <select name="id_position" id="id_position" class="form-control" required>
                            ...
                            <option value="<?= Encryption::encode($pos['id']) ?>"
                                <?= $employee->id_position == $pos['id'] ? 'selected' : ''  ?>>
                                <?= $pos['title'] . ' - ' . $pos['department']  ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="id_boss" required>
                            ...
                            <option value="<?= Encryption::encode($type_pos['id_employee']) ?>"
                                <?= $employee->id_boss == $type_pos['id_employee'] ? 'selected' : ''  ?>>
                    ...
                    <div class="divReasion" <?= isset($employee->end_date) ? '' : 'hidden' ?>>
                        <div class="form-group ">
                            ...
                            <input type="date" name="end_date" id="end_date" class="form-control end_date2"
                                value="<?= isset($employee->end_date) ? $employee->end_date : ''  ?>"
                                <?= $employee->status == 0 ? 'required' : '' ?>>
                        </div>

                        <label for="reason_for_leaving" class="col-form-label">Razón de baja</label>
                        <select class="form-control" name="reason_for_leaving" id="selectreason_for_leaving"
                            <?= $employee->status == 0 ? 'required' : '' ?>>
                            <option disabled selected value="">Selecciona razon de baja</option>
                            <option value="Abandono del empleo"
                                <?= $employee->reason_for_leaving == 'Abandono del empleo' ? 'selected' : '';  ?>>
                                Abandono del empleo</option>
                            <option value="Baja con causal"
                                <?= $employee->reason_for_leaving == 'Baja con causal del empleo' ? 'selected' : '';  ?>>
                                Baja con causal</option>
                            <option value="Cambio de residencia"
                                <?= $employee->reason_for_leaving == 'Cambio de residencia' ? 'selected' : '';  ?>>
                                Cambio de residencia</option>
                            <option value="Recorte de personal"
                                <?= $employee->reason_for_leaving == 'Recorte de personal' ? 'selected' : '';  ?>>
                                Recorte de personal</option>
                            <option value="Reestructura de la empresa"
                                <?= $employee->reason_for_leaving == 'Reestructura de la empresa' ? 'selected' : '';  ?>>
                                Reestructura de la empresa</option>
                            <option value="Renuncia voluntaria"
                                <?= $employee->reason_for_leaving == 'Renuncia voluntaria' ? 'selected' : '';  ?>>
                                Renuncia voluntaria</option>
                        </select>

                        <div class="form-group">
                            <label for="comment_for_leaving" class="col-form-label">Razón de baja</label>
                            <textarea class="form-control comment_for_leaving" name="comment_for_leaving" rows="8"
                                <?= $employee->status == 0 ? 'required' : '' ?>><?= $employee->comment_for_leaving  ?> </textarea>
                        </div>
                    ...
                    <div class="form-group divRe_entrry_date" <?= isset($employee->re_entry_date) ? '' : 'hidden' ?>>
                        ...
                        <input type="date" name="re_entry_date" id="re_entry_date" class="form-control "
                            value="<?= isset($employee->re_entry_date) ? $employee->re_entry_date : ''  ?>"
                            <?= $employee->status == 0 ? 'required' : '' ?>>
                    </div>
