            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-left mb-2">
                        ...
                        <li class="breadcrumb-item"><a href="<?=base_url?>empresa_SA/ver&id=<?=Encryption::encode($cliente->Empresa)?>"><?=$cliente->Nombre_Empresa?></a></li>
              ...
              <div class="col-md-12">
                            ...
                            <div id="content-facturacion">
                                <div class="row">
                                    <div class="col text-center">
                                        ...
                                        <p><?=$cliente->Corte_Servicio == 1 ? 'Contraentrega' : ($cliente->Corte_Servicio == 2 ? 'Semanal' : ($cliente->Corte_Servicio == 3 ? 'Quincenal' : ($cliente->Corte_Servicio == 4 ? 'Mensual' : 'Sin asignar')))?></p>
                                    </div>
                                    <div class="col text-center">
                                        ...
                                        <p><?=$cliente->Corte_Servicio == 1 ? '' : ($cliente->Corte_Servicio == 2 ? 'Cada '.$cliente->Fechas_Especificas : ($cliente->Corte_Servicio == 3 ? 'Los días '.explode(',', $cliente->Fechas_Especificas)[0].' y '.explode(',', $cliente->Fechas_Especificas)[1] : ($cliente->Corte_Servicio == 4 ? 'El día '.$cliente->Fechas_Especificas : '')))?></p>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col text-center">
                                ...
                                <div class="row">
                                    <div class="col text-center">
                                        <b>Uso de portales</b>
                                        <p><?=$cliente->Uso_Portal == 0 ? 'Sí' : ($cliente->Uso_Portal == 1 ? 'No' : '')?></p>
                                    </div>
                                    <div class="col">
                                        <b>Dirección</b>
                ...
                <div class="card card-info">
                    ...
                    <div class="card-body">
                        ...
                        <?php if (Utils::isAdmin() || Utils::isManager() || Utils::isSales() || Utils::isSalesManager() || Utils::isSenior()): ?>
                            <div class="text-center">
                                <button class="btn btn-info" id="btn-modificar-razones">Añadir o quitar razones</button>
                            </div>
                        <?php endif ?>
                    </div>
                </div>
                <?php if (Utils::isAdmin() || Utils::isManager() || Utils::isSales() || Utils::isSalesManager() || Utils::isSenior() || Utils::isSAManager() || Utils::isSalesManager() || Utils::isOperationsSupervisor() || Utils::isLogisticsSupervisor() || Utils::isAccount()): ?>
                    <div class="card card-navy">
                        ...
                        <div class="card-body">
                            <div id="content-cuentas">
                                <div class="row">
                                    <div class="col-md-6 text-center">
                                        <b>Contacto</b>
                                        <p><?=$cliente->Cuentas_Contacto?></p>
                                    </div>
                                    <div class="col-md-6 text-center">
                                        <b>Dirección de correo electrónico</b>
                                        <p><?=$cliente->Cuentas_Correo?></p>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-8 text-center">
                                        <b>Teléfono</b>
                                        <p><?=$cliente->Cuentas_Telefono?></p>
                                    </div>
                                    <div class="col-md-4 text-center">
                                        <b>Extensión</b>
                                        <p><?=$cliente->Cuentas_Extension?></p>
                                    </div>
                                </div>
                            </div>
                            <?php if (Utils::isAdmin() || Utils::isManager() || Utils::isSales() || Utils::isSalesManager() || Utils::isSenior()): ?>
                                <div class="text-center">
                                    <button class="btn btn-info" id="btn-editar-cuentas">Editar</button>
                                </div>
                            <?php endif ?>
                        </div>
                    </div>    
    ...
    document.addEventListener('DOMContentLoaded', e => {
        ...
        document.querySelector('#tb_contacts').addEventListener('click', e => {
            if (e.target.classList.contains('btn-info') || e.target.offsetParent.classList.contains('btn-info')) {
                ...
                $('#modal_contacto').modal({backdrop: 'static', keyboard: false});// inciiogaboooooooooooooooo
        ...
        document.querySelector('#tb_razones').addEventListener('click', e => {
            if (e.target.classList.contains('btn-info') || e.target.parentElement.classList.contains('btn-info')) {
                ...
                $('#modal_razon').modal({backdrop: 'static', keyboard: false});
...
<script type="text/javascript">
    ...
    document.querySelector('#btn-editar-condiciones').addEventListener('click', e => {
        ...
        $('#modal_condiciones').modal({backdrop: 'static', keyboard: false});
    })
    ...
    document.querySelector('#btn-editar-facturacion').addEventListener('click', e => {
        ...
        $('#modal_facturacion').modal({backdrop: 'static', keyboard: false});
    })

    document.querySelector('#btn-editar-cuentas').addEventListener('click', e => {
        e.preventDefault();
        let cliente = new Cliente();
        cliente.getCuentas(<?=$cliente->Cliente?>);
        $('#modal_cuentas').modal({backdrop: 'static', keyboard: false});
    })
    ...
    document.querySelector('#btn-editar-comentario').addEventListener('click', e => {
        ...
        $('#modal_comentario').modal({backdrop: 'static', keyboard: false});
    })
    ...
    document.querySelector('#btn-nuevo-contacto').addEventListener('click', e => {
        ...
        document.getElementById("Usuario").removeAttribute("readonly");  // incio gabo
    ...
    document.querySelector('#btn-modificar-razones').addEventListener('click', e => {
        ...
        $('#modal_razones').modal({backdrop: 'static', keyboard: false});
    })
