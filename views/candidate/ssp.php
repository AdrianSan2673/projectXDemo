...
class SSP
{
     ...
     */
    static function simple($request, $conn, $table, $primaryKey, $columns, $botones, $extraWhere = '', $extraFields = '')
    {
        $bindings = array();
        $db = self::db($conn);
        ...
        if ($extraWhere) $extraWhere = ($where) ? $extraWhere : ' WHERE ' . $extraWhere;
        // var_dump($extraWhere);
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            // $data[$i]['aptitudes'] = Utils::lineBreak($data[$i]['aptitudes']);
            // $data[$i]['experiences'] = Utils::lineBreak($data[$i]['experiences']);
            $data[$i]['created_at'] = Utils::getShortDate($data[$i]['created_at']);
            $data[$i]['first_name'] = $data[$i]['first_name'] . " " . $data[$i]['surname'] . " " . $data[$i]['last_name'];
        }
        ...
        //si la tabla_postulate se agregan estos campos
        if ($extraFields != '') {
