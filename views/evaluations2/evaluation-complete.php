                         ...
                         <div class="col-sm-6">
                             <ol class="breadcrumb float-sm-left mb-2">
                                 ...
                                 <?php if (Utils::isCustomerSA() && isset($_GET['id'])) : ?>
                                     <li class="breadcrumb-item"><a href="<?= base_url ?>evaluacionempleado/index2&id_group=<?= Encryption::encode($evaluation_employee->id_group_evaluation) ?>">Evaluaciones</a></li>
                                 <?php endif ?>
                             ...
                             <div class="card card-success">
                                 <div class="card-body">
                                     <div class="callout callout-info">
                                         ...
                                         <p>Muchas gracias por haber realizado la evaluación de <?= $employee->first_name . " " . $employee->surname ?></p>
                                     </div>
                                 </div>
                                                     ...
                                                     <div class="row">
                                                         <div class="col-md-11">
                                                             <a class="d-block w-100 collapsed" data-toggle="collapse" href="#collapse_<?= $evaluacionCat['id'] ?>" aria-expanded="true">
                                                                 ...
                                                                 <?php if ($evaluation_employee->score != '') : $score_category = " " . $calificacion[$evaluacionCat['id']] . "%   de ";
                                                                    else :  $score_category = '';
                                                                    endif; ?>
                                                                 <?= $evaluacionCat['category'] . " " . $score_category . number_format(100 / count($evaluationCategory)) . '%' ?>
                                                                                                ...
                                                                                                foreach ($questionsRows as $questRow) : ?>
                                                                                                 ...
                                                                                                 <tr>
                                                                                                     ...
                                                                                                     <?php for ($i = 0; $i < count($criterionScoreColums); $i++) : ?>
                                                                                                         <td class="align-middle text-center"><?= $criterionScoreColums[$i]['value'] == $answer ? "<i class='fas fa-circle text-info text-lg'></i>" : '' ?></td>
                                                                                                     <?php endfor; ?>
                                         ...
                                         <form id="form_open_questions">
                                             <?php foreach ($openQuestions as $openquestion) : ?>
                                                 <div class="form-group">
                                                     ...
                                                     <textarea class="form-control" name="question<?= $openquestion['id']  ?>" maxlength="200" rows="4" required></textarea>
                                                 </div>
                                             <?php endforeach; ?>
                                                                 ...
                                                                 <tbody>
                                                                     <?php
                                                                        ...
                                                                        foreach ($calificaciones  as $categoria => $cal) :   ?>
                                                                         <tr>
                                                                             <td class="text-center"><a><?= $categoria  ?></a></td>
                                                                             <?php if (($cal * 100 / (number_format(100 / count($evaluationCategory)))) >= 70) : ?>
                                                                                 <td class="text-center"><span> <?= $cal . "%" ?> </span></td>
                                                                             <?php else : ?>
                                                                                 <td class="text-center"><span> <?= $cal . "%" ?> </span></td>
                                                                             <?php endif; ?>
                                                                             <td class="text-center"><?= number_format(100 / count($evaluationCategory)) . "%" ?> </td>
                                                                     ...
                                                                     <tr>
                                                                         ...
                                                                         <?php
                                                                            ...
                                                                            if ($total >= 70) : ?>
                                                                             <td class="text-center"><b><span class="badge badge-success"> <?= $total . "%" ?> </span></b></td>
                                                                         <?php else : ?>
                                                                             <td class="text-center"><b><span class="badge badge-warning"> <?= $total . "%" ?> </span></b></td>
                                                                         <?php endif; ?>
                                             ...
                                             <?php if ($evaluation_employee->status == 2) : ?>
                                                 <div class="form-group">
                                                     ...
                                                     <input type="email" class="form-control" name="email_input" id="email_input" maxlength="100" hidden>
                                                     <select class="form-control" name="email_boss" id="email_employee" required>
                                                         <option selected disabled value="">Selecciona correo correo</option>
                                                         <option value="">Escribir email</option>
                                                         <option value="<?= Encryption::encode($email_Employee->email)  ?>" <?= isset($email_Employee->email) && $email_Employee->email != '' ? '' : 'hidden' ?>><?= $email_Employee->email  ?></option>
                                                         <option value="<?= Encryption::encode($email_Employee->institutional_email)  ?>" <?= isset($email_Employee->institutional_email) && $email_Employee->institutional_email != '' ? '' : 'hidden' ?>><?= $email_Employee->institutional_email  ?></option>
                                                     </select>
                                                 </div>
                                                 <input type="hidden" name="id_evaluation_employee" value="<?= Encryption::encode($id) ?>">
                                                 <input type="submit" name="submit" class="btn btn-info float-right" value="Retroalimentar">
                                             <?php endif ?>
                                         </form>
                                     </div>
                                 </div>
