...
require_once 'helpers/cv-templates/Resume1.php';
require_once 'helpers/cv-templates/Resume2.php';
require_once 'helpers/cv-templates/Resume3.php';
require_once 'helpers/cv-templates/Resume5.php';
require_once 'helpers/cv-templates/Resume6.php';
require_once 'helpers/cv-templates/Resume7.php';
require_once 'helpers/cv-templates/Resume8.php';
require_once 'helpers/cv-templates/Resume9.php';
require_once 'helpers/cv-templates/Resume10.php';
require_once 'helpers/cv-templates/Resume11.php';
require_once 'helpers/cv-templates/Resume12.php';
require_once 'helpers/cv-templates/Resume13.php';
require_once 'helpers/cv-templates/Resume14.php';
require_once 'helpers/cv-templates/Resume15.php';
require_once 'helpers/cv-templates/Resume16.php';
require_once 'helpers/cv-templates/Resume17.php';
require_once 'helpers/cv-templates/Resume18.php';
require_once 'helpers/cv-templates/Resume19.php';
require_once 'helpers/cv-templates/Resume20.php';
require_once 'helpers/cv-templates/Resume21.php';
require_once 'helpers/cv-templates/Resume22.php';
require_once 'helpers/cv-templates/Resume23.php';
require_once 'helpers/cv-templates/Resume24.php';
...
class ResumeController{
	...
	public function psicometria(){
		if (true) {
            ...
            $pdf->SetTitle("Psicometria - " . mb_strtoupper($psycho->candidate, 'UTF-8'), true);
			...
			$pdf->Output('I', "Psicometria - " . mb_strtoupper($psycho->candidate, 'UTF-8') . '.pdf', true);
		}else{
    ...
    public function creator()
    {
        if (isset($_POST) && isset($_SESSION['data'])) {
            ...
            $red = isset($_SESSION['data']->red) ? $_SESSION['data']->red : 51;
            $green = isset($_SESSION['data']->green) ? $_SESSION['data']->green : 54;
            $blue = isset($_SESSION['data']->blue) ? $_SESSION['data']->blue : 79;
            $candidato = [];
            $candidato =
                (object) array(
                    'id' => $_SESSION['data']->id,
            ...
            $route = isset($_SESSION['route']) && !empty($_SESSION['route']) ? $_SESSION['route']/* Utils::getImage($_SESSION['route'])[0] */ : "dist/img/user-icon.png";
            $apt_lang = array_merge($preparations, $languages);

            $pdf = new $template("P", "pt", "Letter");
            require('./libraries/fpdf/makefont/makefont.php');
            $pdf->AddFont('SinkinSansLight', '', 'SinkinSans-300Light.php');
            $pdf->AddFont('SinkinSans', '', 'SinkinSans-400Regular.php');
            $pdf->AddFont('SinkinSans', 'I', 'SinkinSans-400Italic.php');
            ...
            $pdf->AddPage();
            $pdf->SetAbout($candidato, $route);
            if (in_array(get_class($pdf), ['Resume2', 'Resume3', 'Resume5', 'Resume6', 'Resume7', 'Resume8', 'Resume12', 'Resume16', 'Resume17', 'Resume18', 'Resume23']))
                $pdf->setExperience($experiences, $candidato);
            else
                $pdf->SetExperience($experiences);

            $pdf->SetEducation($candidato->education, $apt_lang);
            $pdf->setAdditionalPreparation($preparations);
            if (in_array(get_class($pdf), ['Resume9', 'Resume15']))
            $pdf->setLanguages($languages, $candidato);
            else
                $pdf->setLanguages($languages);
            $pdf->SetAptitude($aptitudes);
    ...
    public function selectColor() {
        if (isset($_POST) && isset($_SESSION['data'])) {
            $red = isset($_POST['red']) ? $_POST['red'] : 51;
            $green = isset($_POST['green']) ? $_POST['green'] : 54;
            $blue = isset($_POST['blue']) ? $_POST['blue'] : 79;
            $_SESSION['data']->red = $red;
            $_SESSION['data']->green = $green;
            $_SESSION['data']->blue = $blue;
        }
    }
