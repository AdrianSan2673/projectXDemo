...
class FormatoController
{
    ...
    public function ese()
    {
        if (isset($_GET['candidato']) && !empty($_GET['candidato'])) {
            ...
            $videollamada = base_url . 'ServicioApoyo/previa&candidato=' . Encryption::encode($_GET['candidato']);

            QRcode::png($videollamada, $dir . $qrFile, 'M', 8, 6);
            $pdf->qr = $dir . $qrFile;
            //$pdf->Enlace_Drive = $candidato_datos->Enlace_Drive;
            $pdf->Enlace_Drive = $videollamada;
            //}
            $pdf->AliasNbPages();
    ...
    public function soiqr()
    {
        if (isset($_GET['candidato']) && !empty($_GET['candidato'])) {
            ...
            imagepng($soiImage->image);
            imagedestroy($soiImage->image);
        } else
            header("location:" . base_url);
    }
