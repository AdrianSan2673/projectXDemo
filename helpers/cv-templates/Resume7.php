	...
	public function setAbout($candidato, $route){
		...
		if (!is_null($candidato->description) && !empty($candidato->description)) {
			if (strlen($candidato->description) <= 300) {
				$this->Rect($xa, $ya-10, 204, 511, 'F');
			}elseif (strlen($candidato->description) > 300 && strlen($candidato->description) < 400) {
				$this->Rect($xa, $ya-10, 204, 590, 'F');
			}else{
				$this->Rect($xa, $ya-10, 204, 670, 'F');
			}
		}else{
			$this->Rect($xa, $ya-10, 204, 511, 'F');
		}
		...
		$this->SetFillColor(250, 234, 235);
		$this->Rect(0, 57, 650, 800, 'F');

		
		$this->SetFillColor(47, 61, 126);
	
		$this->Rect(0, 0,225, 80, 'F');
		...
		if (!is_null($candidato->description) && !empty($candidato->description)) {
			...
			if (!is_null($candidato->telephone) && !empty($candidato->telephone)) {
				$y = $this->GetY() + 10;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv7/phone.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->email) && !empty($candidato->email)) {
				$y += 15;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv7/email3.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->state) && !empty($candidato->state)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv7/location3.png', $xa+5, $y, 8);
			...
			if (isset($candidato->linkedinn) && !is_null($candidato->linkedinn) && !empty($candidato->linkedinn)) {
				...
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv7/linkedin.png', $xa+5, $y, 8);
			...
			if (isset($candidato->facebook) && !is_null($candidato->facebook) && !empty($candidato->facebook)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv7/facebook.png', $xa+5, $y, 8);
			...
			if (isset($candidato->instagram) && !is_null($candidato->instagram) && !empty($candidato->instagram)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv7/instagram.png', $xa+5, $y, 8);
	...
	public function setLanguages($languages){
		if (count($languages) > 0) {
			...
			foreach ($languages as $value => $lang) {
				...
				for ($i=1; $i <=10 ; $i++) {
					if ($i <= $aux) :
						$this->Image('dist/img/resume-icons/cv7/bsquareB.png', $x + $ln, $this->GetY() , 8);
					

					elseif ($i == 10) :
						$this->Image('dist/img/resume-icons/cv7/squareEnd.png', $x +$ln, $this->GetY() , 8);
		
					else :
						$this->Image('dist/img/resume-icons/cv7/squareMid.png', $x + $ln, $this->GetY() , 8);
