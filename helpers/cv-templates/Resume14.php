...
class Resume14 extends FPDF
{
	...
	public function setAbout($candidato, $route){
		...
		$this->SetFillColor(45, 52, 60); 
		$this->SetDrawColor(45, 52, 60);
		$this->ClippingRoundedRect(400,15,205,145,5,true);
		$this->Rect(400, 10,205, 150, 'F');
		$this->UnsetClipping();

		$this->ClippingRoundedRect(400,170,205,600,5,true);
		$this->Rect(400, 170,205, 600, 'F');
		$this->UnsetClipping();

		$this->SetLineWidth(5);	
		$this->ClippingRoundedRect(20,30,90,100,5,true);
		$this->Image($route, 20, 30, 90, 100);
		$this->UnsetClipping();

		$this->setFont('Lato-Bold', 'B', 13);
		$this->setTextColor(1,1,1);
		$this->setXY(120, $y);
		$this->MultiCell(350, 15, utf8_decode(mb_strtoupper($candidato->first_name.' '.$candidato->surname.' '.$candidato->last_name)), 0,'L');
		
		$this->setFont('Lato-Regular', '', 10);
		$y = $this->GetY() + 2;
		$this->setXY(120, $y);
		$this->setTextColor(205, 182, 145);
		...
		if (!is_null($candidato->description) && !empty($candidato->description)) {
			...
			$this->setFont('Lato-Regular', '', 10);
			$this->MultiCell(260, 11.5, utf8_decode($candidato->description), 0, 'J');
		}
			...
			if (!is_null($candidato->telephone) && !empty($candidato->telephone)) {
				$y = $this->GetY() + 10;
				$this->setXY(420, $y);
				$this->Image('dist/img/resume-icons/cv14/phone.png', $xa+390, $y, 8);
			...
			if (!is_null($candidato->email) && !empty($candidato->email)) {
				$y += 15;
				$this->setXY(420, $y);
				$this->Image('dist/img/resume-icons/cv14/email3.png', $xa+390, $y, 8);
			...
			if (!is_null($candidato->state) && !empty($candidato->state)) {
				$y = $this->GetY() + 8;
				$this->setXY(420, $y);
				$this->Image('dist/img/resume-icons/cv14/location3.png', $xa+390, $y, 8);
			...
			if (isset($candidato->linkedinn) && !is_null($candidato->linkedinn) && !empty($candidato->linkedinn)) {
				...
				$this->setXY(420, $y);
				$this->Image('dist/img/resume-icons/cv14/linkedin.png', $xa+390, $y, 8);
			...
			if (isset($candidato->facebook) && !is_null($candidato->facebook) && !empty($candidato->facebook)) {
				$y = $this->GetY() + 8;
				$this->setXY(420, $y);
				$this->Image('dist/img/resume-icons/cv14/facebook.png', $xa+390, $y, 8);
			...
			if (isset($candidato->instagram) && !is_null($candidato->instagram) && !empty($candidato->instagram)) {
				$y = $this->GetY() + 8;
				$this->setXY(420, $y);
				$this->Image('dist/img/resume-icons/cv14/instagram.png', $xa+390, $y, 8);
	...
	public function setLanguages($languages){
		if (count($languages) > 0) {
			...
			foreach ($languages as $value => $lang) {
				...
				for ($i=1; $i <=10 ; $i++) {
					if ($i <= $aux) :
						$this->Image('dist/img/resume-icons/cv14/bsquareB.png', $x + $ln, $this->GetY() , 8);
					

					elseif ($i == 10) :
						$this->Image('dist/img/resume-icons/cv14/squareEnd.png', $x +$ln, $this->GetY() , 8);
		
					else :
						$this->Image('dist/img/resume-icons/cv14/squareMid.png', $x + $ln, $this->GetY() , 8);
