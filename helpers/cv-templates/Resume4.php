...
class Resume extends FPDF
{
	...
	public function setAbout($candidato, $route){
		...
		$this->SetFillColor(7, 155, 182);/* Azul */
		$this->Rect(0, 0, 650, 800, 'F');
		
		
		$this->SetFillColor(64, 64, 64); /* Gris */
		$this->Rect(0, 25,125, 145, 'F');
		...
		if (!is_null($candidato['description']) && !empty($candidato['description'])) {
			...
			if (!is_null($candidato['telephone']) && !empty($candidato['telephone'])) {
				$y = $this->GetY() + 5;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv4/phoneInv.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato['email']) && !empty($candidato['email'])) {
				$y += 15;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv4/emailInv.png', $xa+5, $y, 8);
				$this->MultiCell(180, 10, utf8_decode($candidato['email']), 0, 'L');
			}

			if (!is_null($candidato['state']) && !empty($candidato['state'])) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv4/locationInv.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato['linkedinn']) && !empty($candidato['linkedinn'])) {
				...
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv4/linkedinInv.png', $xa+5, $y, 8);
				//$this->Cell(80, 10, ($candidato['linkedinn']), 0, 1, 'L');
				$this->MultiCell(180, 10, utf8_decode($candidato['linkedinn']),0, 'L');
			}

			if (!is_null($candidato['facebook']) && !empty($candidato['facebook'])) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv4/facebookInv.png', $xa+5, $y, 8);
				//$this->Cell(80, 10, ($candidato['description']), 0, 1, 'L');	
				$this->MultiCell(180, 10, utf8_decode($candidato['facebook']),0, 'L');
			}

			if (!is_null($candidato['instagram']) && !empty($candidato['instagram'])) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv4/instagramInv.png', $xa+5, $y, 8);
				//$this->Cell(80, 10, ($candidato['instagram']), 0, 1, 'L');
				$this->MultiCell(180, 10, utf8_decode($candidato['instagram']),0, 'L');
			}
