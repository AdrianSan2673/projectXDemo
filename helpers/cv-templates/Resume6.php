...
class Resume6 extends FPDF
{
	...
	public function setAbout($candidato, $route){
		...
		$this->ClippingCircle(110,90,70,70,false);
		$this->Image($route, 35, 20, 150,150);
		$this->UnsetClipping();
		$this->SetLineWidth(1);
		...
		if (!is_null($candidato->description) && !empty($candidato->description)) {
			...
			if (!is_null($candidato->telephone) && !empty($candidato->telephone)) {
				$y = $this->GetY() + 10;
				$this->setXY($xa+35, $y);
				$this->Image('dist/img/resume-icons/cv6/phoneInv.png', $xa+25, $y, 8);
			...
			if (!is_null($candidato->email) && !empty($candidato->email)) {
				$y += 15;
				$this->setXY($xa+35, $y);
				$this->Image('dist/img/resume-icons/cv6/emailInv.png', $xa+25, $y, 8);
			...
			if (!is_null($candidato->state) && !empty($candidato->state)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+35, $y);
				$this->Image('dist/img/resume-icons/cv6/locationInv.png', $xa+25, $y, 8);
			...
			if (isset($candidato->linkedinn) && !is_null($candidato->linkedinn) && !empty($candidato->linkedinn)) {
				...
				$this->setXY($xa+35, $y);
				$this->Image('dist/img/resume-icons/cv6/linkedinInv.png', $xa+25, $y, 8);
			...
			if (isset($candidato->facebook) && !is_null($candidato->facebook) && !empty($candidato->facebook)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+35, $y);
				$this->Image('dist/img/resume-icons/cv6/facebookInv.png', $xa+25, $y, 8);
			...
			if (isset($candidato->instagram) && !is_null($candidato->instagram) && !empty($candidato->instagram)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+35, $y);
				$this->Image('dist/img/resume-icons/cv6/instagramInv.png', $xa+25, $y, 8);
...
/* 			$this->SetFillColor(1,14, 111);
			...
			foreach ($aptitudes as $value => $apt) {
				...
				for ($i=1; $i <=10 ; $i++) {
					if ($i <= $apt['level']) {
						$this->Image('dist/img/resume-icons/cv6/circleB.png', $x + $ln, $this->GetY() + 8, 8);
					} else {
						$this->Image('dist/img/resume-icons/cv6/circleemptyB.png', $x + $ln, $this->GetY() + 8, 8);
					}
					$ln += 9.5;
				}
