...
class Resume21 extends FPDF
{
	...
	public function setAbout($candidato, $route){
		...
		$this->SetFillColor(196, 217, 223);  /* Gris */
		$this->Rect(0, 0, 190,800, 'F');
		$this->UnsetClipping();



		$this->SetFillColor(16, 101, 126); /* Azul */
		...
		$this->setXY(190,30);
		$this->MultiCell(450, 17, utf8_decode(mb_strtoupper($candidato->first_name.' '.$candidato->surname.' '.$candidato->last_name)),0,'C');

		$y = $this->GetY() +5;
		$this->setFont('NotoSans-Regular', '', 12);
		$this->setXY(190,$y);
		$this->MultiCell(450, 12, utf8_decode(mb_strtoupper($candidato->job_title)), 0, 'C');
		if (!is_null($candidato->description) && !empty($candidato->description)) {
			...
			$this->setTextColor(255,2552,255);
			...
			if (!is_null($candidato->telephone) && !empty($candidato->telephone)) {
				$y = $this->GetY() + 150;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv21/phone.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->email) && !empty($candidato->email)) {
				$y += 15;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv21/email3.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->state) && !empty($candidato->state)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv21/location3.png', $xa+5, $y, 8);
				$this->MultiCell(155, 10, utf8_decode($candidato->city.', '.$candidato->state), 0, 'L');	
			}
			...
			if (isset($candidato->linkedinn) && !is_null($candidato->linkedinn) && !empty($candidato->linkedinn)) {
				...
				$xa+= 170;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv21/linkedin.png', $xa+5, $y, 8);
			...
			if (isset($candidato->facebook) && !is_null($candidato->facebook) && !empty($candidato->facebook)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv21/facebook.png', $xa+5, $y, 8);
			...
			if (isset($candidato->instagram) && !is_null($candidato->instagram) && !empty($candidato->instagram)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv21/instagram.png', $xa+5, $y, 8);
