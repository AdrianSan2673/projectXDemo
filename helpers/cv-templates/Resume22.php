...
class Resume22 extends FPDF
{
	...
	public function setAbout($candidato, $route){
		...
		$this->SetFillColor(28, 59, 79);  /* Naranja */
		$this->Rect(0, 0, 190,230, 'F');
		$this->UnsetClipping();

		$this->SetFillColor(231, 118, 42);  /* Naranja */
		$this->Rect(0, 230, 190,800, 'F');
		$this->UnsetClipping();

	

		$this->SetDrawColor(231, 118, 42);
		$this->SetLineWidth(1);
		$this->SetFillColor(231, 118, 42);
		$this->ClippingPolygon(array(190,0,190,230,0,230),true);
		...
		if (!is_null($candidato->description) && !empty($candidato->description)) {
			...
			if (!is_null($candidato->telephone) && !empty($candidato->telephone)) {
				$y = $this->GetY() + 150;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv22/phone.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->email) && !empty($candidato->email)) {
				$y += 15;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv22/email3.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->state) && !empty($candidato->state)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv22/location3.png', $xa+5, $y, 8);
			...
			if (isset($candidato->linkedinn) && !is_null($candidato->linkedinn) && !empty($candidato->linkedinn)) {
				...
				$xa+= 170;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv22/linkedin.png', $xa+5, $y, 8);
			...
			if (isset($candidato->facebook) && !is_null($candidato->facebook) && !empty($candidato->facebook)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv22/facebook.png', $xa+5, $y, 8);
			...
			if (isset($candidato->instagram) && !is_null($candidato->instagram) && !empty($candidato->instagram)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv22/instagram.png', $xa+5, $y, 8);
