...
class Resume19 extends FPDF
{
	...
	public function setAbout($candidato, $route){
		...
		$this->Rect(0, 0, 189,800, 'F');
		$this->UnsetClipping();



		$this->SetFillColor(16, 101, 126); /* Azul */
		$this->Rect(189, 0, 650,231, 'F');

		$this->SetDrawColor(16, 101, 126);
		$this->SetLineWidth(1);
		$this->SetFillColor(16, 101, 126);
		$this->ClippingPolygon(array(0,0,189,0,189,230),true);
		$this->Rect(0, 0, 650,230, 'F');
		$this->UnsetClipping();

		$this->Image($route,24,30,140,170);
		...
		if (!is_null($candidato->description) && !empty($candidato->description)) {
			...
			if (!is_null($candidato->telephone) && !empty($candidato->telephone)) {
				$y = $this->GetY() + 150;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv19/phone.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->email) && !empty($candidato->email)) {
				$y += 15;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv19/email3.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->state) && !empty($candidato->state)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv19/location3.png', $xa+5, $y, 8);
			...
			if (isset($candidato->linkedinn) && !is_null($candidato->linkedinn) && !empty($candidato->linkedinn)) {
				...
				$xa+= 170;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv19/linkedin.png', $xa+5, $y, 8);
			...
			if (isset($candidato->facebook) && !is_null($candidato->facebook) && !empty($candidato->facebook)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv19/facebook.png', $xa+5, $y, 8);
			...
			if (isset($candidato->instagram) && !is_null($candidato->instagram) && !empty($candidato->instagram)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv19/instagram.png', $xa+5, $y, 8);
