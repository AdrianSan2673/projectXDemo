...
class Resume16 extends FPDF
{
	...
	public function setAbout($candidato, $route){
		...
		if (!is_null($candidato->description) && !empty($candidato->description)) {
			$this->setTextColor(7, 44, 97);
			$this->setFont('NotoSans-Bold', 'B', 11);
			/* $y = $this->GetY() + 20; */
			$this->setXY($xa+215,70);
			...
			if (!is_null($candidato->telephone) && !empty($candidato->telephone)) {
				$y = $this->GetY() + 4;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv16/phoneInv.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->email) && !empty($candidato->email)) {
				$y += 15;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv16/emailInv.png', $xa+5, $y, 8);
			...
			if (!is_null($candidato->state) && !empty($candidato->state)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv16/locationInv.png', $xa+5, $y, 8);
			...
			if (isset($candidato->linkedinn) && !is_null($candidato->linkedinn) && !empty($candidato->linkedinn)) {
				...
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv16/linkedinInv.png', $xa+5, $y, 8);
			...
			if (isset($candidato->facebook) && !is_null($candidato->facebook) && !empty($candidato->facebook)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv16/facebookInv.png', $xa+5, $y, 8);
			...
			if (isset($candidato->instagram) && !is_null($candidato->instagram) && !empty($candidato->instagram)) {
				$y = $this->GetY() + 8;
				$this->setXY($xa+15, $y);
				$this->Image('dist/img/resume-icons/cv16/instagramInv.png', $xa+5, $y, 8);
	...
	public function setExperience($experiencia,$candidato){
		...
		if (count($experiencia) > 0) {
			...
			$this->setX(235);
			$this->Cell(60, 10, utf8_decode('EXPERIENCIA'), 0,'L');
