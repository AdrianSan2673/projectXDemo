...
class Reclutamiento extends FPDF
{

	public function setText	($prospecto, $cuota, $garantia){
		$x = 87;
		$y = 146;
		...
		$this->Cell(0, 15, 'Reclutamiento. ', 0, 1, 'L');
		...
		$y = $this->GetY() + 30;
		$this->setXY($x, $y);
		$this->Cell(0, 15, '*  Envío de candidatos hasta cubrir la posición', 0, 1, 'L');
		$this->setX($x);
		$this->Cell(0, 15, '*  Garantía de '.$garantia.' por renuncia del candidato.', 0, 1, 'L');
		$this->setX($x);
		$this->Cell(0, 15, '*  Sin anticipos. ', 0, 1, 'L');

		$y = $this->GetY() + 30;
		$this->setXY($x, $y);
		$this->Cell(128, 15, 'El costo del servicio será de', 0, 1, 'L');
		$this->setXY($this->GetX() + 215, $this->GetY()-15);
		$this->setFont('SinkinSans', 'B', 8);
		$this->Cell(5, 15, $cuota.'%', 0, 1, 'L');
		$this->setXY($this->GetX() + 238, $this->GetY()-15);
		$this->setFont('SinkinSans', '', 8);
		$this->Cell(0, 15, ' del valor de 1 mes de sueldo por la vacante.', 0, 1, 'L');
