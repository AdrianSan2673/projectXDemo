...
class ESEP extends FPDF
{
	...
	public function setDatosPersonales($candidato){
		...
		if($candidato->RFC!='' || $candidato->RFC!=null)
		{
		$this->setFont('SinkinSans', 'B', 6.5);
		$this->setTextColor(140, 140, 140);
		$this->SetXY(397, $y);
	...
	public function setDatosContacto($candidato, $domicilio){
		...
		if ($candidato->Telefono_fijo) {
			...
			$this->setTextColor(140, 140, 140);
			$this->SetXY($x1 + 150, $y);
			$this->MultiCell(50, 18, utf8_encode('Otro'), 0, 'L', false);
...
public function setEstudios($escolaridad, $comentarios)
	...
	public function setCohabitantes($cohabitantes, $comentarios){
		...
		$this->setXY(293, $y); */
		$this->MultiCell(195, 20, utf8_encode('Ocupación'), 0, 'C', false);
		$this->setXY(392, $y+5);
		...
		foreach ($cohabitantes as $cohabitante) { 
			...
			$this->MultiCell(205, 8, utf8_encode($cohabitante['Nombre']), 0, 'C', false);
			$this->setXY(239, $y);
			...
			$this->MultiCell(175, 8, utf8_encode($cohabitante['Edad_2'] == 'Años' && $cohabitante['Edad'] > 2 ? $cohabitante['Ocupacion'] : 'No aplica'), 0, 'C', false);
			$this->setXY(403, $y);
			
			$this->MultiCell(100, 8, utf8_encode($cohabitante['Edad_2'] == 'Años' && $cohabitante['Edad'] > 2 ? $cohabitante['Empresa'] : 'No aplica'), 0, 'C', false);
			$this->setXY(450, $y);
	...
	public function setVivienda($vivienda, $ubicacion, $comentarios){
		if ($vivienda && $ubicacion) {
			...
			*/
			...
			if($vivienda->Domicilio_es!='' || $vivienda->Domicilio_es!=null){
			$this->setFont('SinkinSans', 'B', 6.5);
			$this->setTextColor(140, 140, 140);
			$this->setXY(310, $y-15);
			$this->MultiCell(140, 15, utf8_encode('El domicilio donde vive es'), 0, 'L', false);
...
*/
		...
		$this->SetXY(10, $y);
		$this->MultiCell(591, 16, utf8_encode('Infromacion confiable'), 0, 'L', true);
		$this->SetFillColor(255, 255, 255);
	...
	public function setReferenciasLaborales($referencias_laborales, $Cliente, $Empresa){
		foreach ($referencias_laborales as $key => $referencia) { 
			...
			$this->setTextColor(140, 140, 140);
			$this->SetXY(321, $y+3);
			$this->MultiCell(103, 18, utf8_encode('Web'), 0, 'L', false);
			...
			*/
			...
			$this->setTextColor(140, 140, 140);
			$this->SetXY($x, $y);
			$this->MultiCell(103, 9, utf8_encode('Nombre de quien proporciona la información'), 0, 'L', false);

			$this->setFont('SinkinSans', '', 6.5);
			$this->setTextColor(0, 0, 0);
			$this->setXY(130, $y);
			$this->MultiCell(176, 27, utf8_encode($referencia['Informante']), 0, 'L', true);
			
		    $this->setFont('SinkinSans', 'B', 6.5);
			$this->setTextColor(140, 140, 140);
			$this->SetXY(321, $y);
			$this->MultiCell(103, 9, utf8_encode('Puesto de quien proporciona la información'), 0, 'L', false);
